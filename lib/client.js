...
export default class Client {
  /**
   * Return a new `Client` object for the first Roku device discovered on the network.
   * See `./discover.js` for function signature, as params are forwarded directly to that function.
   * @return {Promise<Client>} A `Client` object.
   */
  static discover(...args) {
    return discover(...args).then(ip => new Client(ip));
  }
